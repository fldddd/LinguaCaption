...
class SubtitleResult:
    """字幕结果"""
    def __init__(self, language: str, full_text: str, segments: List[SubtitleSegment], raw: Dict = None):
...
class BilibiliSubtitleFetcher:
    ...
    def _get_cid(self, bvid: str) -> Optional[int]:
        ...
        try:
            resp = httpx.get(url, params={"bvid": bvid}, headers=self._headers(), timeout=10)
            data = resp.json()
        except Exception as e:
            print(f"获取 cid 失败: {e}")
            return None
        if data.get("code") != 0:
            print(f"view API 返回错误: code={data.get('code')}, msg={data.get('message')}")
            return None
    ...
    def _list_subtitles(self, bvid: str, cid: int) -> List[dict]:
        ...
        try:
            resp = httpx.get(url, params={"bvid": bvid, "cid": cid}, headers=self._headers(), timeout=10)
            data = resp.json()
        except Exception as e:
            print(f"获取字幕列表失败: {e}")
            return []
        if data.get("code") != 0:
            print(f"player API 返回错误: code={data.get('code')}, msg={data.get('message')}")
            return []
    ...
    def _fetch_body(self, subtitle_url: str) -> Optional[List[dict]]:
        try:
            resp = httpx.get(self._normalize_url(subtitle_url), headers=self._headers(), timeout=15)
            data = resp.json()
            return data.get("body") or []
        except Exception as e:
            print(f"下载字幕 JSON 失败: {e}")
            return None

    def fetch_subtitles(self, video_url: str) -> Optional[SubtitleResult]:
        bvid = extract_video_id(video_url, "bilibili")
        if not bvid:
            print("无法从 URL 提取 BV id")
            return None

        cid = self._get_cid(bvid)
        if not cid:
            print(f"{bvid} 没有取到 cid")
            return None

        subtitles = self._list_subtitles(bvid, cid)
        if not subtitles:
            print(f"{bvid} (cid={cid}) 没有可用字幕轨")
            return None

        track = self._pick(subtitles)
        if not track or not track.get("subtitle_url"):
            print(f"{bvid} 字幕轨存在但没有 subtitle_url（可能未登录、需要 SESSDATA cookie）")
            return None
        ...
        print(f"B站直拉字幕成功: {bvid} lan={lan} 共 {len(segments)} 段")
