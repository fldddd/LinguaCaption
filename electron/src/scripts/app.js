...
function initEventListeners() {
  ...
  // 内嵌悬浮窗口控制
  const btnInlineOverlayToggle = document.getElementById('btn-inline-overlay-toggle');
  if (btnInlineOverlayToggle) {
    btnInlineOverlayToggle.addEventListener('click', async () => {
      const videoContainer = document.getElementById('video-container');
      const audioContainer = document.getElementById('audio-container');
      const container = videoContainer || audioContainer;

      if (!container) {
        showToast('当前页面不支持内嵌悬浮窗', 'warning');
        return;
      }

      // 动态导入 inlineOverlay 模块
      try {
        const { initInlineOverlay, toggleInlineOverlay, isInlineOverlayVisible, updateInlineSubtitle } = await import('./inlineOverlay.js');

        if (!isInlineOverlayVisible()) {
          // 初始化并显示
          initInlineOverlay(container);
          btnInlineOverlayToggle.classList.add('active');

          // 监听字幕更新，实时同步到内嵌悬浮窗
          if (window.subtitleUpdateCallback) {
            window.subtitleUpdateCallback = null;
          }
          window.subtitleUpdateCallback = (text) => {
            updateInlineSubtitle(text);
          };

          showToast('内嵌悬浮窗已开启', 'success');
        } else {
          // 隐藏
          toggleInlineOverlay();
          btnInlineOverlayToggle.classList.remove('active');
          showToast('内嵌悬浮窗已关闭', 'success');
        }
      } catch (err) {
        console.error('Inline overlay error:', err);
        showToast('内嵌悬浮窗初始化失败', 'error');
      }
    });
  }
}
