...
function syncLoop() {
  ...
  // 更新内嵌悬浮窗口的字幕（如果有的话）
  // 同时隐藏字幕面板，让字幕只显示在悬浮窗口中
  if (window.subtitleUpdateCallback && active) {
    window.subtitleUpdateCallback(active.text);

    // 隐藏字幕面板
    const subtitlePanel = document.getElementById('subtitle-panel') || document.getElementById('subtitle-panel-point');
    if (subtitlePanel) {
