...
本项目通过飞书群聊 `oc_33c9e0399cd59fd6a2012218ec8aff26` 中 4 个 Agent 协作开发：
...
2. **超时接管**：Agent 沉默 30 分钟无响应 → Architect 立即接管其任务
3. **看板驱动**：使用 Kanban 看板 `linguacaption` 追踪任务进度
4. **状态同步**：每次提交更新 `docs/REQUIREMENTS.md`
5. **任务依赖**：遵循任务依赖链，阻塞任务及时标记并通知上下游
