...
| Agent | 角色 | 部门 | 上级 | 职责 |
|-------|------|------|------|------|
| 🚀 **创业者** | 创业者（CEO） | — | — | 顶层管理：商业战略、创新驱动、第一性原理、市场定位 |
| 🤖 **Hermes Agent** | 技术开发部主管 | 技术开发部 | 创业者 | 管理技术团队、架构决策、代码审核、任务分派、开发实施 |
| 📋 **PM** | 产品经理 | 产品创意部 | 创业者 | 需求定义、用户调研、功能优先级排序、产品路线图 |
| 📐 **执行策划** | 执行策划 | 产品创意部 | 创业者 | 执行方案设计、任务拆解、时间线管理、资源协调 |
| 🏗️ **Architect** | 架构师 | 技术开发部 | Hermes Agent | 系统设计、技术方案评审 |
| 💻 **Developer** | 开发者 | 技术开发部 | Hermes Agent | 编码实现、功能开发、Bug 修复 |
| 👀 **Reviewer** | 审查者 | 技术开发部 | Hermes Agent | 代码审查、测试覆盖、质量把关 |
| 🔧 **DevOps** | 运维 | 技术开发部 | Hermes Agent | CI/CD、基础设施、环境配置、文档维护 |

### 协作规则

1. **@mention 格式**：Agent 间引用必须用 `@open_id` 格式（如 `@ou_aae937ad9b3a9d03c889924c518c0685`），不能用纯文本 @名字
2. **垂直汇报**：技术开发部向 Hermes Agent 汇报 → Hermes Agent 向创业者汇报；产品创意部向创业者直接汇报
3. **跨部门协作**：产品创意部定义需求 → 创业者审批 → Hermes Agent 分派给技术开发部实现，通过看板流转
4. **超时接管**：Agent 沉默 30 分钟无响应 → 其直属上级接管任务（技术线→Hermes Agent，产品线→创业者）
5. **看板驱动**：使用 Kanban 看板 `linguacaption` 追踪任务进度
6. **状态同步**：每次提交更新 `docs/REQUIREMENTS.md`
7. **任务依赖**：遵循任务依赖链，阻塞任务及时标记并通知上下游
